...
int main() {
    ...
    while(T--) {
        ...
        if(N == 1) {
            cout << (((s2[0] != 0) ? 0 : 2) + ((K == 1) ? 1 : 0)) << endl;
            continue;
        }

        vector<char> a1(s1.size(), 0); // assignment (0=red, 1=ref, 2=blue)

        unordered_map<int, unordered_map<char, int> > m;

        for(int it = 0; it < pow(3, a1.size()); ++it) { // each student can be either red, blue or referee

            int d, r;
            tie(d, r) = dot(a1, s1);

            for(char &ea : a1) {
            	ea += 1; // change role from red to referee, and so on
            	ea %= 3; // only 3 roles
                if(ea != 0) // increment next only when current becomes zero 
                	break;
            }
