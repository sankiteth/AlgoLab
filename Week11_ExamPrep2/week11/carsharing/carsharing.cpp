...
void testcase(){
	...
	const int MAXP = 100;
	const int INF = MAXL*S; // Infinite capacity.

	vector<Booking> B;
	vector<set<int> > times(S); // Sets for the coordinate compression per station.
	for (int s = 0; s < S; ++s) { // Add sentinels.
		times[s].insert(0);
		times[s].insert(MAXT);
	}
	...
	for (int i = 0; i < N; ++i) {
		int s, t, d, a, p;
		cin >> s >> t >> d >> a >> p;
		s--; t--; // Convert to zero-based times.
		times[s].insert(d); times[t].insert(a);
		B.push_back({s,t,d,a,p});
	}

	vector<int> psum(S+1); // Prefix sums of coordinate lengths.
	for (int s = 0; s < S; ++s) {
		int i = 0; // Create the coordinate mapping for station s.
		for (auto &t : times[s]) {
			M[s][t] = i;
			i++;
		}
	...
	int T = psum.back(); // # Vertices = total number of timesteps + 2.
	int v_source = T;
	int v_target = T+1;
	Graph G(T+2);
	...
	for (int i = 0; i < S; ++i) { // Add the edges for the timelines.
	...
	for(int i=0; i<N; i++) { // Add the booking edges.
