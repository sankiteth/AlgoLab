...
bool has_interference(delaunay_t const & trg, K::FT const & rr)
{
	for (auto e = trg.finite_edges_begin(); e != trg.finite_edges_end(); ++e){
		if (trg.segment(*e).squared_length() <= rr){
			return true;
		}
	}
...
bool try_two_color(delaunay_t & trg, K::FT const & rr)
{
	...
	for (auto v = trg.finite_vertices_begin(); v != trg.finite_vertices_end(); ++v)
	{
		if (v->info().first == 0) // if vertes unvisited
		{
			v->info().first = ++components;
			std::vector<vhandle_t> stack(1, v);
		
			while (!stack.empty()) // iterative DFS using stack
			{
				...
				{
					if (c->info() == h->info()){ // both vertices have same color, so 2-coloring of graph not possible
						return false;
					}
					if (c->info().first == 0) // still not visited
					{
						stack.push_back(c);
						c->info() = { components, !h->info().second }; // assign alternate color
					}
				}
		...
		if (v->info().second) // create new triangulations for both colors to ensure no interference among same color vertices
			trg1.insert(v->point());
		else 
			trg0.insert(v->point());
	}
...
void test_case()
{
	...
	for (std::size_t i = 0; i < m; ++i)
	{
		...
		if (success) // a network without interferences possible
		{
			if (CGAL::squared_distance(holmes, watson) <= rr) // both holmes and watson within each other's range
			{
				std::cout << "y"; 
				continue;
			}
			...
			if (station_holmes->info().first == station_watson->info().first &&  // both belong to same component
				CGAL::squared_distance(holmes, station_holmes->point()) <= rr && // holmes within range of a station
				CGAL::squared_distance(watson, station_watson->point()) <= rr)   // watson within range of a station
			{
				std::cout << "y"; 
				continue;
			}
